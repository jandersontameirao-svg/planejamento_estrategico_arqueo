...
import { Building2, BarChart3, Zap, Users, Target, TrendingUp, AlertCircle, Lightbulb, ChevronDown, ChevronUp, FileDown, SlidersHorizontal, DollarSign } from "lucide-react";
...
export default function PlanejamentoEstrategicoParticipacoes() {
  const [expandedCards, setExpandedCards] = useState<{ [key: string]: boolean }>({});
  const [dialogMetodologias, setDialogMetodologias] = useState(false);

  // Usar empresaId = -1 para representar o Grupo Arqueo Participações
  const PARTICIPACOES_ID = -1;

  // Metodologias ativas para Participações
  const { data: metodologiasAtivas, refetch: refetchMetodologias } = trpc.metodologias.getByEmpresa.useQuery(
    { empresaId: PARTICIPACOES_ID }
  );
  ...
  const { data: pestelData } = trpc.analises.getPestel.useQuery({ empresaId: PARTICIPACOES_ID });
  const { data: swotData } = trpc.analises.getSwot.useQuery({ empresaId: PARTICIPACOES_ID });
  const { data: okrData } = trpc.analises.getOkr.useQuery({ empresaId: PARTICIPACOES_ID });
  const { data: bscData } = trpc.bsc.getByEmpresa.useQuery({ empresaId: PARTICIPACOES_ID });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <PageHeaderWithBack
        title="Planejamento Estratégico - Grupo Arqueo Participações"
        description="Defina e acompanhe as análises estratégicas do Grupo Arqueo Participações"
        backUrl="/"
      />
            ...
            <DialogContent className="max-w-5xl w-[92vw] max-h-[90vh] flex flex-col gap-0 p-0 overflow-hidden">
              <DialogHeader className="px-6 pt-6 pb-0 flex-shrink-0">
                <DialogTitle className="flex items-center gap-2 text-lg">
                  ...
                  Configurar Metodologias — Grupo Arqueo Participações
                </DialogTitle>
              </DialogHeader>
        ...
        {analisesFiltradas.map((analise) => {
          ...
          return (
            <Card key={`expanded-${analise.id}`} className="mt-4 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <CardContent>
                <Componente empresaId={PARTICIPACOES_ID} />
              </CardContent>
            </Card>
          );
        })}
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Visualize gráficos e métricas consolidadas de todas as análises estratégicas do Grupo Arqueo Participações
                </CardDescription>
              </div>
              <Link href="/dashboard-analises-participacoes">
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Gere um PDF com todas as análises estratégicas do Grupo Arqueo Participações
                </CardDescription>
              </div>
