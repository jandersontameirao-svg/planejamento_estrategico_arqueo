...
import {
  ...
  DollarSign, BarChart3, CalendarDays, Info,
} from "lucide-react";
...
export default function CapitalGiro() {
    ...
    return (
      <DashboardLayout>
        <div className="p-8 text-center text-muted-foreground">
          Empresa não encontrada. Acesse via menu de navegação.
        </div>
      </DashboardLayout>
    );
  }
  ...
  if (error) {
    return (
      <DashboardLayout>
        <div className="p-8 text-center text-destructive">Erro ao carregar: {error.message}</div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                {empresa?.nome ?? `Empresa #${id}`} — Ciclo de Conversão de Caixa
              </p>
            </div>
          </div>
        ...
        <Tabs value={abaAtiva} onValueChange={setAbaAtiva}>
          ...
          {/* Aba Visão Geral — gráfico de evolução */}
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    {
                      label: "CCC Médio",
                      value: (historico.reduce((s, r) => s + r.ccc, 0) / historico.length).toFixed(1),
                    },
                    {
                      label: "CCC Mínimo",
                      value: Math.min(...historico.map((r) => r.ccc)).toFixed(1),
                    },
                    {
                      label: "CCC Máximo",
                      value: Math.max(...historico.map((r) => r.ccc)).toFixed(1),
                    },
                  ].map((stat) => (
                    <Card key={stat.label}>
